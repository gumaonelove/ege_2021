...
for _ in range(N):
    ...
    s_new = [None] * M
    for k in range(M):
        variatns = [x + s[(k-x)%M] for x in numbers if s[(k-x)%M] is not None]
        s_new[k] = min(variatns) if variatns else None
    s[:] = s_new
